...
// TODO: Implementation. RTD Aug'12
...
{
  /// <summary>
  ///   Implementation of a general Bates stochastic volatility model.
  /// </summary>
  /// <remarks>
  ///   <para>The <see cref="Heston">Heston model</see> is a well known stochastic volatility model named after
  ///     Steven Heston. See "A Closed-Form Solution for Options with Stochastic Volatility with Applications
  ///     to Bond and Currency Options", by Steven L. Heston, ''The Review of Financial Studies''
  ///     1993 Volume 6, number 2, pp. 327-343 [http://www.jstor.org/pss/2962057].</para>
  ...
  ///   </list>
  ///   <para><b>Bates</b></para>
  ///   <para>The Bates model is a popular extension of the Heston model where a jump component is added to
  ///     the asset process.</para>
  ///   <math>
  ///     dX_t = \mu S_t dt + \sqrt{v_t} S_t dW^{X}_{t} + dZ_t
  ///   </math><math>
  ///     dv_t = k\left ( \theta - v_t \right ) dt + \xi \sqrt{v_t} dW^v_{t}
  ///   </math>
  ///   <para>where:</para>
  ///   <list type="bullet">
  ///     <item><span><m>Z</m> is a compound Poison process with Gaussian jumps</span></item>
  ///   </list>
  /// </remarks>
  public class Bates
  {
    /// <summary>
    /// Initializes a new instance of the <see cref="Bates"/> class.
    /// </summary>
    ...
    public Bates(Func<double[], double> payoffFn, int simulations, int steps, double T, double s0, double r,
