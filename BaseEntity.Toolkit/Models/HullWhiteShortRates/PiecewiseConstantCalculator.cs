...
namespace BaseEntity.Toolkit.Models.HullWhiteShortRates
{
  ...
  /// </summary>
  public class PiecewiseConstantCalculator
  {
    #region Constructor
    ...
    ///    \\ b_k &amp;= q_{k}/q_{k+1}\qquad k = i,\ldots, j-1
    ///    \\ c_k &amp;= K\,\delta(T_{k-1},T_k) + 1 - b_k\qquad k = i+1,\ldots, j - 1
    ///    \\ c_j &amp;= 1 + K\,\delta(T_{j-1},T_j)
    ///    \\ X_k &amp;= \exp\!\left(A(T_i,T_k) - B(T_i, T_k)\,r^*\right)
    ///  \end{align}</math>
    ///  where <m>r^*</m> solves<math>
    ///     \sum_{k=i+1}^j c_k\,\exp\!\left(A(T_i,T_k) - B(T_i,T_k)\,r^*\right) = b_{i}
    ///  </math>and <m>q_k</m>'s are the ratios of the projection factors
    ///  to the corresponding discount factors.<math>
    ///    q_k \equiv \frac{\mathrm{ProjectionFactor}(t, T_k)}{\mathrm{DiscountFactor}(t, T_k)}
    ///  </math>
    /// </summary>
    ...
    /// <param name="basis">The integral of the basis spread</param>
    ...
    public double EvaluateSwaptionPayer(int i, int j,
      ...
      IReadOnlyList<double> basis = null)
    {
      var solver = new Brent2();
      solver.setToleranceF(1E-15);
      solver.setToleranceX(1E-15);
      return EvaluateSwaptionPayer(i, j,
        strike, zeroPrices, time, fractions, basis, solver,
        new double[Count], new double[Count], new double[Count]);
    }

    /// <summary>
    /// Evaluates the swaption payer.
    /// </summary>
    /// <param name="i">The index of the forward swap starting date</param>
    /// <param name="j">The index of the forward swap maturity date</param>
    ...
    /// <param name="solver">The solver for the at the money short rate</param>
    /// <param name="arrayA">The array a.</param>
    /// <param name="arrayB">The array b.</param>
    /// <param name="arrayC">The array of fraction by end dates</param>
    /// <returns>swaption value</returns>
    public double EvaluateSwaptionPayer(
      ...
      Solver solver,
      double[] arrayA, double[] arrayB, double[] arrayC)
    {
      Debug.Assert(i >= -1 && i < j && j < Count);
      Debug.Assert(zeroPrices != null && zeroPrices.Count == Count);
      Debug.Assert(time != null && zeroPrices.Count == Count);
      Debug.Assert(fractions != null && fractions.Count == Count);
      Debug.Assert(basis == null || basis.Count == Count);
      Debug.Assert(arrayA != null && arrayA.Length == Count);
      Debug.Assert(arrayB != null && arrayB.Length == Count);
      Debug.Assert(arrayC != null && arrayC.Length == Count);

      for (int k = i + 1; k <= j; ++k)
      {
        arrayB[k] = CalculateB(i, k);
        arrayA[k] = CalculateA(i, k, zeroPrices, time);
        arrayC[k] = strike*fractions[k] + (
          basis != null && k < j ? (1 - basis[k]/basis[k + 1]) : 0);
      }
      arrayC[j] += 1;
      var yTarget = basis != null ? (basis[i]/basis[i + 1]) : 1.0;
      ...
      var rstar = solver.solve(f, null, yTarget, initialTrial);
