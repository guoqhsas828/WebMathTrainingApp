...
namespace BaseEntity.Toolkit.Ccr
{
  ...
  /// 
  /// </summary>
  public class PykhtinRosenExposureAggregator : CCRExposureAggregatorManaged
  {
    #region Data
    ...
    private readonly SimulatedPathValues[] _paths;
    ...
    private static readonly ILog Logger = LogManager.GetLogger(typeof(PykhtinRosenExposureAggregator));
    ...
    public PykhtinRosenExposureAggregator(
      Dt asOf,
      ...
      )
    {
      ...
      _paths = new SimulatedPathValues[exposures.PathCount];
    ...
    private PykhtinRosenExposureAggregator(PykhtinRosenExposureAggregator other, Tuple<Dt[], double[]>[] kernels)
    {
    ...
    /// <summary>
    /// Boolean type. Diagnostices supported or not.
    /// </summary>
    public override bool DiagnosticsSupported
    {
      get { return _binaryLoggingEnabled; }
    }
    #endregion
    ...
    public override double[] GetMeasureMarginal(CCRMeasure measure, int t, double ci)
    {
      ...
      if (isIntegral)
      {
        ...
        if (useCVAWeights)
        {
          ...
          {
            if(allocatedValues[i] != 0 && totalCVA != 0)
    ...
    public override CCRExposureAggregator ChangeIntegrationKernels(IList<Tuple<Dt[], double[]>> kernels)
    {
      ...
      var newAgg = new PykhtinRosenExposureAggregator(this, newKernels);
      return newAgg;
    }
    ...
    private void RunSimulationPath(int idx, CCRMeasureAccumulator accumulator, out double[,] pathDiagnostics)
    {
      ...
      for (int t = 0; t < DateCount; ++t)
      {
        ...
        resultData.Add(nettingSetExposures, _df[idx][t], _numeraire[idx][t], _cptyRn[idx][t], _ownRn[idx][t], _survivalRn[idx][t], _cptySpread[idx][t], _ownSpread[idx][t], _lendSpread[idx][t], _borrowSpread[idx][t], idx, 1.0);

        var eeColl = CounterpartyExposure.ComputeCollateralAmounts(resultData, resultData, t, nettingSetExposures, null);
        var neeColl = BookingEntityExposure.ComputeCollateralAmounts(resultData, resultData, t, nettingSetExposures, null);
        ...
        pathEE.Path = pathNEE.Path = resultData;
        pathEE.PathIdx = pathNEE.PathIdx = idx;

        accumulator.AccumulateExposures(pathEE, pathNEE);
        ...
        {
          // Positive Exposure
          pathDiagnostics[0, t] = pathEE.Exposure;
          // Negative Exposure
          pathDiagnostics[1, t] = pathNEE.Exposure;
          // Received Collateral
          pathDiagnostics[2, t] = pathEE.Collateral;
          // Posted Collateral
          pathDiagnostics[3, t] = pathNEE.Collateral;
        }
      }
    }
    ...
    {
      var cptyTotals = new double[PathCount][][];
      var bookingTotals = new double[PathCount][][];
      for (int p = 0; p < PathCount; p++)
      {
        cptyTotals[p] = new double[DateCount][];
        bookingTotals[p] = new double[DateCount][];
        Parallel.For(0, DateCount, (d) =>
        {
          ...
          cptyTotals[p][d] = CounterpartyExposure.CollateralizedTotals(_nettingSetExposures[p][d], _nettingSetRcvCollateral[p][d]);
          bookingTotals[p][d] = BookingEntityExposure.CollateralizedTotals(_nettingSetExposures[p][d], _nettingSetPayCollateral[p][d]);
        });
      }
      ...
      Parallel.For(0, nettingCount, (n) =>
      {
        ...
        var cptyTotalsForNettingSet = new double[PathCount][];
        var bookingTotalsForNettingSet = new double[PathCount][];
        var nettingSetExposures = new double[PathCount][];
        Parallel.For(0, PathCount, (p) =>
        {
          cptyTotalsForNettingSet[p] = new double[DateCount];
          bookingTotalsForNettingSet[p] = new double[DateCount];
          nettingSetExposures[p] = new double[DateCount];
          for (int d = 0; d < DateCount; ++d)
          {
            cptyTotalsForNettingSet[p][d] = cptyTotals[p][d][n];
            bookingTotalsForNettingSet[p][d] = bookingTotals[p][d][n];
            nettingSetExposures[p][d] = _nettingSetExposures[p][d][n];
          }
        });
        ...
        {
          var tradePvs = PrecalculatedPvs.GetExposures(_reversePortfolioMap[n][t], ExposureDates);
          for (int p = 0; p < PathCount; p++)
          {
            for (int d = 0; d < DateCount; ++d)
            {
              ...
              double ownWeight = _ownRn[p][d];
              double df = _df[p][d];
              var v_i = tradePvs[p, d];
              var ee = cptyTotalsForNettingSet[p][d];
              var nee = bookingTotalsForNettingSet[p][d];
              var v = nettingSetExposures[p][d];
              var marginalV = v.AlmostEquals(0.0) ? 0.0 : v_i / v;
              _allocatedEeWeights[n][t][d] += marginalV * ee * df * cptyWeight;
              _allocatedNeeWeights[n][t][d] += marginalV * nee * df * ownWeight;
              _allocatedEpvWeights[n][t][d] += v_i * df;
            }
          }
        });
      });
