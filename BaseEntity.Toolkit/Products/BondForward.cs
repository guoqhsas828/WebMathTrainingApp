...
{
  ///<summary>
  /// Bond Forward product
  ///</summary>
  /// <remarks>
  ///   <para>A bond forward is an OTC contract where the holder has the obligation to purchase or sell a
  ...
  ///   <h1 align="center"><img src="Forward_Payoff_Long.png" width="70%" align="middle"/></h1>
  ///   <para>For a short position this is <m>F_T = K - S_T</m></para>
  ///   <h1 align="center"><img src="Forward_Payoff_Short.png" width="70%" align="middle"/></h1>
  ...
  [Serializable]
  [ReadOnly(true)]
  public class BondForward : Product
  {
    #region Constructors
    ...
    /// Constructor
    /// </summary>
    /// <param name="bond">Underlying bond</param>
    ...
    public BondForward(Bond bond, Dt fixingDate, Dt deliveryDate)
    ...
    public override void Validate(ArrayList errors)
    {
      ...
      if (Bond == null)
        InvalidValue.AddError(errors, this, "Bond", String.Format("Missing underlying bond"));
      else
      {
        if (Dt.Cmp(LastDeliveryDate, Bond.Effective) < 0)
          InvalidValue.AddError(errors, this, "Expiration", String.Format("Last delivery date {0} must be on or after effective date {1}", LastDeliveryDate, Bond.Effective));
        if (Dt.Cmp(LastDeliveryDate, Bond.Maturity) >= 0)
          InvalidValue.AddError(errors, this, "Expiration", String.Format("Last delivery date {0} must be before underlying maturity {1}", LastDeliveryDate, Bond.Maturity));
        Bond.Validate(errors);
      }
    }
    ...
    {
      var obj = (BondForward)base.Clone();
      obj.Bond = (Bond)Bond.Clone();
      return obj;
    }
