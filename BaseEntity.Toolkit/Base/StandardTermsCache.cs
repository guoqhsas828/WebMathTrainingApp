...
namespace BaseEntity.Toolkit.Base
{
  ...
  ///   <para>Wraps <see cref="ConcurrentDictionary{TKey,T}"/>.</para>
  /// </remarks>
  public abstract class StandardTermsCache<T> : IStandardTermsCache<T> where T : IStandardTerms
  {
    ...
    public void Initialise(Action<IStandardTermsCache<T>> initFunc = null)
    {
      if( initFunc != null )
        _initFunc = initFunc;
      _terms = null;
    ...
    /// </summary>
    /// <remarks>
    ///   <note>Does not force re-loading of cache.</note>
    /// </remarks>
    public void Clear()
    {
      if (_terms == null)
    ...
    /// <returns>True if cache contains specified key, false otherwise</returns>
    ...
    public void Add(T terms)
    {
      if (!Terms.TryAdd(terms.Key, terms))
        throw new ArgumentException($"Attempting to add duplicate key {terms.Key}");
      return;
    }

    /// <summary>
    /// Add set of terms to cache
    /// </summary>
    /// <param name="terms">Terms to add</param>
    /// <exception cref="ArgumentException">Duplicate terms key already defined</exception>
    /// <exception cref="ArgumentNullException">Key is null</exception>
    /// <exception cref="OverflowException">Max number of elements has been reached</exception>
    public void Add(IEnumerable<T> terms)
    {
