...
{
  /// <summary>
  ///  Representing a payment with fixed amount
  /// </summary>
  [DebuggerDisplay("{DebugDisplay}")]
  internal sealed class FixedPaymentExpression : PaymentExpression
    , IStructuralEquatable, IReadOnlyCollection<Evaluable>, IDebugDisplay
  {
    public readonly Evaluable DiscountedAmount;

    public static FixedPaymentExpression Create(
      double amount, Curve discountCurve, Dt payDt)
    {
      return Unique(new FixedPaymentExpression(amount, discountCurve, payDt));
    }

    private FixedPaymentExpression(
      double amount, Curve discountCurve, Dt payDt)
    ...
    {
      var node = other as FixedPaymentExpression;
      return node != null && 
    ...
    public string DebugDisplay
    {
      get
      {
        ...
        return string.Format("PvAt({0}, {1})", PayDt,
          d != null ? d.DebugDisplay : DiscountedAmount.ToString());
      }
    }
