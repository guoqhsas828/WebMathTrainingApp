...
{
  [TestFixture("TestFxFutureOptionBlackPricer01")]
  [TestFixture("TestFxFutureOptionBlackPricer02")]
  [TestFixture("TestFxFutureOptionBlackPricer03")]
  [TestFixture("TestFxFutureOptionBlackPricer04")]
  [TestFixture("TestFxFutureOptionBlackPricer05")]
  public class TestFxFutureOptionBlackPricer : ToolkitTestBase
  {

    public TestFxFutureOptionBlackPricer(string name) : base(name)
    {
    }
     ...
     public void Initialize()
    {
      RcvCcy = Currency.USD;
       PayCcy = Currency.EUR;
       var fxFuture = new FxFuture(RcvCcy, PayCcy, Maturity, 1000000.0, 0.01);
      var option = new FxFutureOption(fxFuture, Expiration, Type, Style, Strike);
      DiscountData = GetDiscountData(AsOf);
      var dc = DiscountData.GetDiscountCurve();
      // Fx
      var fxRate = new FxRate(AsOf, 0, RcvCcy, PayCcy, SpotFxRate, Calendar.None, Calendar.None);
      var fxCurve = new FxCurve(fxRate, new[] { fxFuture.Maturity }, new[] { FwdFxRate }, new[] { "Maturity" }, null);
      _pricer = new FxFutureOptionBlackPricer(option, AsOf, AsOf, dc, fxCurve, CalibratedVolatilitySurface.FromFlatVolatility(AsOf, FlatVol), 1.0);
    ...
    private void TestNumeric(Func<FxFutureOptionBlackPricer, double> method)
    {
      TestNumeric(
        _pricer,
        "",
        obj => method((FxFutureOptionBlackPricer)obj));
    }
    ...
    private FxFutureOptionBlackPricer _pricer;
