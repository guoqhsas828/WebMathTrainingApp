...
{
  /// <summary>
  /// This test will test different inenr CDO maturities for CDO2
  /// </summary>
  [TestFixture]
  public class CDO2DiffInnerMaturitiesTest : ToolkitTestBase
  {
    #region SetUP
    [OneTimeSetUp]
    ...
    public void TestLossToDate()
    {
      ...
      // Make three CDO maturities shorter
      maturities_[3] = new Dt(maturities_[3].Day, maturities_[3].Month - 4, maturities_[3].Year - 1);
      BuildCDO2Pricer(maturities_);
      var lossToDates4 = new double[num];
      for (int i = 0; i < num; i++)
        lossToDates4[i] = Math.Abs(cdo2Pricer_.LossToDate(scheduleDates[i]));
      ...
      for (int i = 0; i < num; i++)
      {
        // If current date is earlier than the shorter CDO maturity, check loss equality
        // If current date is later than the shorter CDO maturity, check "less than"
        less &= (scheduleDates[i] < maturities_[3]
                   ? Math.Round(lossToDates4[i], 4) == Math.Round(lossToDates3[i], 4)
                   : Math.Round(lossToDates4[i], 4) < Math.Round(lossToDates3[i], 4));
      }
      Assert.IsTrue(less, "Loss To Date with shorter CDO maturity are not less than those with same maturity");
    ...
    private void BuildCreditCurves()
    {
      ...
      for(int i = 0; i < survCurves.Length; i++)
      {
        survCurves[i] = SurvivalCurve.FitCDSQuotes(creditNames[i], asOf_, Dt.Empty, Currency.USD, "", false,
                                                   ...
                                                   new[] {"5y"}, new Dt[] {new Dt(20, 6, 2014)}, new[] {quotes[i]},
                                                   new[] {recoveryRates[i]}, 0, new Dt[] {Dt.Empty, Dt.Empty}, null, 0,
                                                   convRecoveryRates[i], discountCurveISDA, true);
      }
      return;
    }
    ...
    {
      var lossLevels = new double[1, 2];
      lossLevels[0, 0] = cdo2_.Attachment;
      lossLevels[0, 1] = cdo2_.Detachment;
      ...
      // Create copula
      var copula = new Copula(CopulaType.Gauss, 5, 5);

      // Create recovery curves
      var rc = new RecoveryCurve[survCurves.Length];
      for (int i = 0; i < survCurves.Length; i++)
      {
        if (survCurves[i].Calibrator != null)
          rc[i] = survCurves[i].SurvivalCalibrator.RecoveryCurve;

      }

      var basket =
        new MonteCarloCDO2BasketPricer(asOf_, settle_, cdo2Maturity_, survCurves, rc, prins, attachments_, detachments_,
                                       false, copula, corr, 3, TimeUnit.Months, lossLevels, sampleSize, 12345, cdoMaturities);

      cdo2Pricer_ = new SyntheticCDOPricer(cdo2_, basket, discountCurve, 1.0, null);
      cdo2Pricer_.Notional = cdo2Notional_;
    ...
    class innerClass
    {
      ...
      public innerClass(int index, double data)
      {
        this.Index = index;
        this.Data = data;
      }
