...
namespace BaseEntity.Metadata
{
  ...
  /// </summary>
  public abstract class ManyToOnePropertyMeta : ScalarPropertyMeta, ICascade
  {
    private static readonly ILog Logger = LogManager.GetLogger(typeof (ManyToOnePropertyMeta));
    ...
    protected ManyToOnePropertyMeta(ClassMeta entity, ManyToOnePropertyAttribute propAttr, PropertyInfo propInfo)
      : base(entity, propAttr, propInfo)
    {
      Clazz = propInfo.PropertyType;
      Fetch = propAttr.Fetch ?? "select";

      OwnershipResolver = GetOwnershipResolver(propAttr.OwnershipResolverType);
      if (OwnershipResolver == null)
      {
        Cascade = propAttr.Cascade ?? "none";
      }
      else
      {
        Cascade = OwnershipResolver.GetCascade(entity.Type);
      }
      ...
      {
        throw new MetadataException($"Invalid Cascade [{Cascade}] for property [{Entity.Name}.{Name}]");
      }
      if (Entity.IsComponent && Cascade.Contains("all-delete-orphan"))
      {
        throw new MetadataException($"Invalid property [{Entity.Name}.{Name}] : Cannot specify Cascade [{Cascade}]");
      }
    ...
    /// <summary>
    /// Gets the ownership resolver.
    /// </summary>
    /// <value>
    /// The ownership resolver.
    /// </value>
    public IOwnershipResolver OwnershipResolver { get; }
    ...
    /// </summary>
    public ClassMeta ReferencedEntity { get; private set; }

    /// <summary>
    /// 
    /// </summary>
    public ICascade InverseCascade { get; private set; }
    ...
    /// 
    /// </summary>
    public Cardinality Cardinality => Cardinality.ManyToOne;
    ...
    private static IOwnershipResolver GetOwnershipResolver(Type type)
    {
      if (type == null) return null;
      var ci = type.GetConstructor(new Type[0]);
      if (ci == null)
      {
        Logger.ErrorFormat("Type {0} does not have default constructor.", type);
        return null;
      }
      var resolver = ci.Invoke(new object[0]) as IOwnershipResolver;
      if (resolver == null)
      {
        Logger.ErrorFormat("Type {0} does not implement IOwnershipResolver interface.", type);
      }
      return resolver;
    }
    ...
    {
      ReferencedEntity = classCache.Find(Clazz);
      if (ReferencedEntity == null)
      {
        throw new MetadataException($"Invalid ReferencedEntity [{Clazz.Name}] for property [{Entity.Name}.{Name}]");
      }

      _isChild = ReferencedEntity.IsChildEntity && Cascade.Contains("all");
    }
    ...
    public override void ThirdPassInit()
    {
      ...
      if (_isChild)
      {
        ...
        {
          if (!ReferencedEntity.OldStyleValidFrom)
          {
      ...
      if (Cascade == "none" &&
          !ReferencedEntity.HasKey &&
          (InverseCascade == null || InverseCascade.Cascade == "none"))
      {
        throw new MetadataException(
          $"Invalid ReferencedEntity [{ReferencedEntity.Name}] for property [{Entity.Name}.{Name}] : Does not define unique business key");
      }

#if MJT
      if (_referencedEntity.IsChildEntity)
      {
        // Only allow reference to ChildEntity if ReferencedEntity is the owner
        if (InverseCascade == null || !InverseCascade.Cascade.Contains("all"))
        {
          throw new MetadataException(string.Format(
            "Invalid ReferencedEntity [{0}] for property [{1}.{2}] : Cannot directly reference ChildEntity",
            _referencedEntity.Name, Entity.Name, Name));
        }
      }
#endif

      //_creator = IsChild
      //  ? snapshotCreatorFactory.GetSnapshotCreator(_referencedEntity.Type)
      //  : snapshotCreatorFactory.GetSnapshotCreator(typeof(ObjectRef));

      //SnapshotCreator = this;
    }
    ...
    public override void Validate(object obj, ArrayList errors)
    {
      ...
      {
        if (!IsNullable)
          InvalidValue.AddError(errors, obj, Name, $"Value of [{Name}] cannot be empty!");
      }
      else
      {
        if (Cascade != "none")
    ...
    public override string Format(object o, string propertyName = "Name")
    {
      ...
      return (choiceItem == null || nameProp == null) ? "" : nameProp.GetValue(choiceItem).ToString();
    }
    ...
    public ICascade FindInverse()
    {
      ...
      foreach (
        var otherCascade in
          cascadeList.Where(
            otherCascade =>
              otherCascade.Cardinality == Cardinality.OneToMany || otherCascade.Cardinality == Cardinality.ManyToMany)
            .Where(otherCascade => otherCascade.JoinColumn == JoinColumn))
      {
        if (otherCascade.ReferencedEntity == Entity)
          return otherCascade;
        if (Entity.IsA(otherCascade.ReferencedEntity))
          return otherCascade;
      }
    ...
    public override bool MapXmlSchema(XmlSchema xmlSchema, XmlSchemaElement element)
    {
      ...
      if (isOwned)
      {
        XsdGenerator.SetReferencedEntityXmlSchemaType(xmlSchema, element, refClassMeta, OwnershipResolver, Entity.Type);

        XsdGenerator.MapClass(xmlSchema, refClassMeta);
        return true;
      }

      return XsdGenerator.SetReferencedEntityKeyXmlSchemaType(xmlSchema, element, refClassMeta, OwnershipResolver, Entity.Type);
    }
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public abstract ObjectRef GetObjectRef(object obj);
  ...
  public class ManyToOnePropertyMeta<T, TValue> : ManyToOnePropertyMeta
    ...
    where TValue : PersistentObject
  {
    ...
    /// <param name="propAttr"></param>
    /// <param name="propInfo"></param>
    public ManyToOnePropertyMeta(ClassMeta entity, ManyToOnePropertyAttribute propAttr, PropertyInfo propInfo)
      : base(entity, propAttr, propInfo)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public override ObjectRef GetObjectRef(object obj)
    {
      return _fieldGetter((T) obj);
    }
  }
